...
public class withdraw extends JInternalFrame {
	...
	private static withdrawSystem objWit = new withdrawSystem(); 
	...
	public withdraw() {
		...
		JLabel lblNewLabel_11 = new JLabel("Withdraw");
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String id = objWit.find(acc_no).get(0).toString();
					String firstname = objWit.find(acc_no).get(1).toString();
					String lastname = objWit.find(acc_no).get(2).toString();
					String balance = objWit.find(acc_no).get(3).toString();
				...
				if( lblNewLabel_10.getText() != ""  ) {
					 ...
					 boolean state1 ;
						...
						try {
							...
							state = objWit.saved(accno, cust_id, date, balance, amount);
							 ...
							 if( state == true ) {
								 ...
								 state1 = objWit.updateSaved( accno , amount );
								 if( state1 == true ) {
									 
									 JOptionPane.showMessageDialog( null, "deposit update!");
									 
									 lblNewLabel_10.setText( objWit.getUpdateBalanceValue(accno) );
